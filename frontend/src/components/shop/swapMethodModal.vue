<template>
  ...
  >
    <div
      class="bg-[#FDF5F1] max-w-3xl w-full p-6 rounded-lg shadow-lg overflow-y-auto max-h-[90vh] relative"
    >
      ...
      <template v-if="!success">
        ...
        <div class="flex gap-4 mb-6">
          <div
            class="w-1/2 p-4 border rounded-lg cursor-pointer"
        ...
        <div v-if="selectedMethod === 'inPerson'" class="mb-6">
          ...
          <textarea
            v-model="inPersonLocation"
            rows="3"
            :class="inputClass(errors.inPersonLocation)"
          ...
          <div
            class="bg-[#FFF8E1] border border-orange-300 font-swap p-4 mt-4 rounded text-sm text-gray-700"
          >
        ...
        <div v-if="selectedMethod === 'delivery'" class="mb-6">
          ...
          <div
            class="bg-[#FFF8E1] border font-swap border-orange-300 p-4 mt-4 rounded text-sm text-gray-700"
          >
        ...
        <button
          class="mt-4 w-full bg-black text-white px-12 py-4 rounded hover:bg-white hover:border-2 hover:border-swapbase hover:text-swapbase"
          @click="submitSwapDetails"
        >
      ...
      <div v-else class="text-center">
        ...
        <div class="flex flex-row justify-center gap-x-10">
          <button
            class="bg-black text-white px-14 py-4 rounded hover:bg-white hover:border-2 hover:border-swapbase hover:text-swapbase"
            @click="close"
          >
            OK
          </button>
          <button
            class="bg-white border-2 text-black px-6 py-4 rounded hover:bg-swapbase hover:text-white"
            @click="goToStatus"
          >
            Check Status
          </button>
...
const submitSwapDetails = () => {
  ...
  // const offeredItems = allProducts.filter((p: any) => offeredIds.includes(p.id))
  // const wantedItem = allProducts.find((p: any) => p.id === wantedId)
